...
export default function AdminPage() {
    ...
    const handleDeleteFood = async (id) => {
        if (!window.confirm("Hapus makanan ini?")) return;
    ...
    const handleEditFoodStock = async (id, currentStock) => {
        ...
        if (newStock !== null && !isNaN(newStock)) {
            ...
            setFoods((prev) =>
                prev.map((f) => (f.id === id ? { ...f, stock: parseInt(newStock) } : f))
            );
        }
    };
                                        ...
                                        {users.map((user) => (
                                            <tr key={user.id} className="hover:bg-gray-50 transition">
                                                ...
                                                <td className="p-4 text-center">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${ROLE_STYLES[user.role] || ""}`}>
                                                        {user.role}
                                                    </span>
                                                </td>
...
function ActionButton({ onClick, icon: Icon, color }) {
    return (
        <button onClick={onClick} className={`p-2 rounded-lg transition hover:brightness-90 ${color}`}>
